...
| 	Disbursement Failed	| 	Triggred when a payment has NOT been successfully disbursed using the recipient's chosen selection	| 
...
| 	Payment Selected  Canceled	| 	triggerred when a payment has been rejected by a recipient	| 
...
| 	Check Disbursement Printed	| 	webhook to merchant after receiving printed webhook from chekbook.io	| 
